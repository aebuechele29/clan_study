...
```{r appendix-h-load}
inc_std_tbl    <- read_csv(here("9_figures", "output", "income_size_standardized.csv"),  show_col_types = FALSE)
wealth_std_tbl <- read_csv(here("9_figures", "output", "wealth_size_standardized.csv"), show_col_types = FALSE)
```
```{r appendix-h1}
inc_std_tbl %>%
  mutate(
    across(-year, ~ round(as.numeric(.x), 3)),
    `HH - Clan (Current)`  = current_hh - current_clan,
    `HH - Clan (Method 1)` = m1_hh      - m1_clan,
    `HH - Clan (Method 2)` = m2_hh      - m2_clan
  ) %>%
  rename(
    Year              = year,
    `HH (Current)`    = current_hh,
    `Clan (Current)`  = current_clan,
    `HH (Method 1)`   = m1_hh,
    `Clan (Method 1)` = m1_clan,
    `HH (Method 2)`   = m2_hh,
    `Clan (Method 2)` = m2_clan
  ) %>%
  select(Year,
         `HH (Current)`, `Clan (Current)`, `HH - Clan (Current)`,
         `HH (Method 1)`, `Clan (Method 1)`, `HH - Clan (Method 1)`,
         `HH (Method 2)`, `Clan (Method 2)`, `HH - Clan (Method 2)`) %>%
  flextable() %>%
  set_caption(caption = "Appendix H1. Income Gini coefficients: unstandardized and size-standardized",
              autonum = FALSE) %>%
  add_header_row(
    values     = c("", "Current (Unstandardized)", "Method 1 (divide by Size)", "Method 2 (divide by sqrt people)"),
    colwidths  = c(1, 3, 3, 3)
  ) %>%
  theme_vanilla() %>%
  bold(part = "header") %>%
...
wealth_std_tbl %>%
  ...
  select(Year,
         ...
         `HH (Method 2)`, `Clan (Method 2)`, `HH - Clan (Method 2)`) %>%
  flextable() %>%
  set_caption(caption = "Appendix H2. Wealth Gini coefficients: unstandardized and size-standardized",
              autonum = FALSE) %>%
  add_header_row(
    values     = c("", "Current (Unstandardized)", "Method 1 (divide by Size)", "Method 2 (divide by sqrt people)"),
    colwidths  = c(1, 3, 3, 3)
