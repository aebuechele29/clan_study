...
As Table 1 shows, the difference between household and clan income Gini coefficients  is `r fmt2(inc_diff_black)` for Black respondents and `r fmt2(inc_diff_nonblack)` for non-Black respondents. For wealth, the difference between household and clan Gini coefficients is `r fmt2(w_diff_black)` for Black respondents and `r fmt2(w_diff_nonblack)` for non-Black respondents. These differences indicate that the unit of analysis has a slightly larger effect on measured wealth inequality by race than on income inequality, with inequality lower when using clans as the unit of analysis, especially for Black respondents. Nonetheless, the direction of the difference is consistent across all sub-groups, with wealth inequality lower than income inequality.
...
make_two_panel_flextable(
  ...
  year_col = year,
  left_hh = r_hh_w_wealth_withhome, left_clans = r_cl_w_wealth_withhome,
  right_hh = r_hh_w_wealth_nohouse,  right_clans = r_cl_w_wealth_nohouse,
  right_label = "Excluding Home Equity", left_label = "Including Home Equity",
  diff_label         = "HH - Clan",      
  overall_label      = "Change in Gap", 
  overall_diff_label = "Excl. - Incl."   
)
...
```{r appendix-f2}
if (all(c("r_hh_unw_wealth", "r_cl_unw_wealth") %in% names(wealth_withhome))) {
  make_two_panel_flextable(
    df = wealth_withhome,
...
Appendix H compares Gini coefficients from the main analyses with two household/ clan size-standardized alternatives. Method 1 divides income or wealth by the number of people in the household, or number of households in the clan. Method 2 divides by the square root of the number of people in the household or clan, following equivalence scale conventions. Results are shown separately for income and wealth. Across all specifications, estimated inequality remains lower at the clan level than at the household level. Differences between clan and household income Ginis are starker using the size-adjusted measures than our standard measures, while they are similar for wealth Ginis.
```{r appendix-h, fig.width=14, fig.height=15}
grid.newpage()
grid.draw(figH)
```
