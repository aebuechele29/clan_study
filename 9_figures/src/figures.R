...
print(doc, target = here("9_figures", "output", "figure1", "Figure1.docx"))
...
print(doc, target = here("9_figures", "output", "figure2", "Figure2A.docx"))
...
# Save combined PDF
ggsave(
  here("9_figures", "output", "figure2", "Figure2B_C_combined.pdf"),
...
print(doc, target = here("9_figures", "output", "figure3", "Figure3A.docx"))
...
ggsave(
  here("9_figures", "output", "figure3", "Figure3B_C_combined.pdf"),
  plot = combined, width = 14, height = 7
)
...
# FIGURE 4 - BY RACE ##TODO: THIS FIGURE ISN'T QUITE RIGHT
# ---- 4A. Race Table (Income) ----
race_gini <- read_csv(here("7_gini_by_race/output/income_race.csv"))
...
race_data <- read_csv(here("7_gini_by_race/output/income_race.csv")) ##TODO: code is reading this in twice
...
# ---- 4C. Wealth Table ----
wealth_gini <- read_csv(here("7_gini_by_race/output/wealth_nohouse_race.csv"))
...
# ---- 4D. Wealth Line Plot ----
wealth_data <- read_csv(here("7_gini_by_race/output/wealth_nohouse_race.csv")) ##TODO: CODE IS READING THIS IN TWICE
yearly_plot <- wealth_data %>%
  filter(year != "ALL") %>%
  mutate(year = as.numeric(year))
...
print(doc, target = here("9_figures", "output", "figure4", "Figure4_Tables.docx"))

# ---- Export Plots (PDF, side by side) ----
combined_plots <- plot_grid(plot_income, plot_wealth, ncol = 2, rel_widths = c(1, 1))

ggsave(
  here("9_figures", "output", "figure4", "Figure4_Plots.pdf"),
